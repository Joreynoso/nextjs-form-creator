...
export default function SubmissionCard({ sub, form, index, status, responses, date }: SubmissionCardProps) {
    ...
    return (
        <div className="rounded-xl border border-border bg-card overflow-hidden transition-shadow hover:shadow-md">
            ...
            {/* ── Header ─────────────────────────────────────────── */}
            <div className="flex flex-wrap items-center justify-between gap-2 px-4 sm:px-5 py-3 border-b border-border bg-muted/30">
                ...
                <dl className="divide-y divide-border/50">
                    {filledFields.map(field => {
                        ...
                        return (
                            <div
                                key={field.id}
                                className="flex flex-col sm:grid sm:grid-cols-[minmax(120px,1fr)_2fr] gap-0.5 sm:gap-x-6 px-4 sm:px-5 py-3 hover:bg-accent/20 transition-colors"
                            >
